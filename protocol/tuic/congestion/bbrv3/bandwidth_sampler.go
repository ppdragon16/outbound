...
type sendTimeState struct {
	...
	// Size of the packet itself. Needed by BBRv3's InflightAtLoss()
	// computation (draft-ietf-ccwg-bbr §5.5.10.2).
...
type congestionEventSample struct {
	...
	// Send states of every lost packet that had a valid snapshot, in order.
	// BBRv3 processes each lost packet individually (draft-ietf-ccwg-bbr
	// §5.2.4 / §5.5.10.2 HandleLostPacket).
	lostSendStates []sendTimeState
...
func (b *bandwidthSampler) OnCongestionEvent(
	...
	if len(ackedPackets) == 0 {
		...
		eventSample.lostSendStates = lostSendStates
		return *eventSample
	}
...
func sentPacketToSendTimeState(sentPacket *connectionStateOnSentPacket, sendTimeState *sendTimeState) {
	*sendTimeState = sentPacket.sendTimeState
	...
	sendTimeState.size = sentPacket.size
}
