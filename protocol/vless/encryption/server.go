...
func (i *ServerInstance) Handshake(conn net.Conn, fallback *[]byte) (*CommonConn, error) {
	...
	if length == 32 {
		...
		crand.Read(c.PreWrite) // always trust yourself, not the client (also prevents being parsed as TLS thus causing false interruption for "native" and "xorpub")
	...
	x25519SKey, _ := ecdh.X25519().GenerateKey(crand.Reader)
