// Package encryption implements the client (and, for test/orchestration
// parity, the server) side of VLESS Post-Quantum Encryption
// (mlkem768x25519plus), ported from mihomo's transport/vless/encryption,
// which is in turn copied and modified from Xray-core.
//
// The wire format is kept byte-compatible with Xray-core so the outbound
// interoperates with existing servers. Dependencies are adapted to this
// module: stdlib crypto/mlkem (Go 1.24+), lukechampine.com/blake3,
// golang.org/x/sys/cpu and the shared pool package.
package encryption
...
func (c *CommonConn) Write(b []byte) (int, error) {
	...
	outBytes := pool.GetBuffer(5 + 8192 + 16)
	defer pool.PutBuffer(outBytes)
	for n := 0; n < len(b); {
		b := b[n:]
		if len(b) > 8192 {
...
func CreatPadding(paddingLens, paddingGaps [][3]int) (length int, lens []int, gaps []time.Duration) {
	...
	for _, y := range paddingGaps {
		gaps = append(gaps, time.Duration(randBetween(int64(y[1]), int64(y[2])))*time.Millisecond)
	}
	return
}
