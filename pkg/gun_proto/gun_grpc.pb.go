...
const (
	GunService_Tun_FullMethodName         = "/GunService/Tun"
	GunService_TunDatagram_FullMethodName = "/GunService/TunDatagram"
)
...
type GunServiceClient interface {
	...
	TunDatagram(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Hunk, Hunk], error)
}
...
func (c *gunServiceClient) TunDatagram(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Hunk, Hunk], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &GunService_ServiceDesc.Streams[1], GunService_TunDatagram_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[Hunk, Hunk]{ClientStream: stream}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type GunService_TunDatagramClient = grpc.BidiStreamingClient[Hunk, Hunk]
...
type GunServiceServer interface {
	...
	TunDatagram(grpc.BidiStreamingServer[Hunk, Hunk]) error
...
func (UnimplementedGunServiceServer) TunDatagram(grpc.BidiStreamingServer[Hunk, Hunk]) error {
	return status.Error(codes.Unimplemented, "method TunDatagram not implemented")
}
...
func _GunService_TunDatagram_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GunServiceServer).TunDatagram(&grpc.GenericServerStream[Hunk, Hunk]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type GunService_TunDatagramServer = grpc.BidiStreamingServer[Hunk, Hunk]
